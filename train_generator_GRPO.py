...
from model_utils import init_GRPO_model_tokenizer_trainer
...
def parse_args():
    ...
    # Experiment settings
    parser.add_argument('--exp_name', type=str, default="llama-31-8b_generator-model_GRPO_finetune_test")
    parser.add_argument('--evaluate_model', type=bool, default=True)
    parser.add_argument('--prompt_template_name', type=str, default="CoT-GRPO")
    parser.add_argument('--quantization', type=bool, default=True)
    parser.add_argument('--fast_inference', type=bool, default=False)
    parser.add_argument('--gpu_utilisation', type=float, default=0.6)

    # Training settings
    parser.add_argument('--base_model_name', type=str, default="meta-llama/Llama-3.1-8B-Instruct")
    parser.add_argument('--train_model_name', type=str, default="trained_models/generators/llama-31-8b_generator-model_3_epoch_rank16")
    parser.add_argument('--unsloth_chat_template', type=str, default="llama")
    parser.add_argument('--max_seq_length', type=int, default=4096)
    parser.add_argument('--max_prompt_length', type=int, default=2048)
    parser.add_argument('--new_lora', type=bool, default=False)
    ...
    parser.add_argument('--base_output_dir', type=str, default='trained_models/GRPO_generators/')
...
def main():
    ...
    output_dir = f"{args.base_output_dir}/{args.exp_name}-GRPO"
    ...
    if args.backend == "unsloth":
        ...
        elif args.prompt_template_name == "CoT-GRPO":
            args.prompt_template = cot_prompt_template_GRPO
        else:
    ...
    reward_functions = [json_reward, label_reward, aif_reward]

    # Initialise model, tokenizer, trainer and modified dataset
    if args.backend == "unsloth":
        model, tokenizer, trainer, ds = init_GRPO_model_tokenizer_trainer(ds, args.prompt_template, reward_functions, output_dir, args)
    else:
        print("GRPO TRAINING CURRENTLY ONLY SUPPORTS UNSLOTH BACKEND - TERMINATING PROGRAM")
        sys.exit()
